...
/**
  ...
  public static LeaveDetails[] leaveHis(final int empID) {
   ...
   /**
  * returns Max. LeaveId value
  * @param leaveid is used to give leavid
  * @return int.
  */
  ...
  public static String approveDeny(final int leaveId, final int empId, final String lstatus, final String mgrcomments) {
    ...
    if (ld != null) {
      ...
      if (lstatus.equals("YES")) {
        ...
        dbStatus = "DENIED";
        sdao().increment(emplyid, noOfDays);
        res = "Leave Rejected";
      } else {
        sdao().increment(emplyid, noOfDays);
        res = "Invalid entry";
  ...
  public static String applyLeave(final int empId,
    ...
    if (emplo != null) {
      ...
      } else if (startDate.compareTo(appliedOn) < 0) {
        s = "start date must be greater than current date";
      } else if (dif < 0) {
